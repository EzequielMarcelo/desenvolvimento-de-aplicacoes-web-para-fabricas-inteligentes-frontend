...
def mostrar_dashboard_principal():
    ...
    with col_metric1:
        st.metric(label="Temperatura Mínima", value=f"{df_temp['Valor'].min():.2f}")
    with col_metric2:
        st.metric(label="Temperatura Máxima", value=f"{df_temp['Valor'].max():.2f}")
    with col_metric3:
    ...
    with col_graph1:
        ...
        fig_linha = go.Figure(data=go.Scatter(x=df_temp['Hora'], y=df_temp['Valor'], mode='lines+markers', line=dict(color='#6a1b9a', width=3), marker=dict(size=8, color='#c85dd1')))
